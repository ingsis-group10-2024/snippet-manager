...
class RuleControllerTest {
    ...
    fun `modify format rules with empty list`() {
        ...
        whenever(ruleService.createOrUpdateRules(rules, RuleTypeEnum.FORMAT, "testUser", authorizationHeader))
            .thenReturn(updatedRules)

        // Act
        val response = ruleController.modifyFormatRule(rules, authorizationHeader, principal)

        // Assert
        assertEquals(ResponseEntity.ok(updatedRules), response)
        verify(ruleService).createOrUpdateRules(rules, RuleTypeEnum.FORMAT, "testUser", authorizationHeader)
    }
    ...
    fun `modify linting rules with empty list`() {
        ...
        val rules = emptyList<RuleDTO>()
        val updatedRules = emptyList<RuleDTO>()
        whenever(ruleService.createOrUpdateRules(rules, RuleTypeEnum.LINT, "testUser", authorizationHeader))
            .thenReturn(updatedRules)

        // Act
        val response = ruleController.modifyLintingRule(rules, authorizationHeader, principal)

        // Assert
        assertEquals(ResponseEntity.ok(updatedRules), response)
        verify(ruleService).createOrUpdateRules(rules, RuleTypeEnum.LINT, "testUser", authorizationHeader)
    }
